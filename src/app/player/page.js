...
export default function PlayerPage() {
  ...
  if (!currentSlide || !gameData) return <div className="bg-black h-screen w-screen text-white flex items-center justify-center">Loading...</div>;
...
function SlideRenderer({ slide, appearance, timing, onNavigate }) {
  ...
  useEffect(() => {
    ...
    if ((slide.type === 'image' || slide.type === 'video') && slide.data.key) {
      getMediaFromIndexedDB(slide.data.key).then(blob => {
        if (active && blob) setMediaUrl(URL.createObjectURL(blob));
      });
    }
    return () => { active = false; if(mediaUrl) URL.revokeObjectURL(mediaUrl); }
  }, [slide.id, slide.type, slide.data.key]);

  return (
    <div className="w-full h-full flex flex-col items-center justify-center relative" style={bgStyle}>
        
      {/* Media Layer */}
      {slide.type !== 'normal' && (
        <div className="absolute inset-0 z-0">
          {slide.type === 'image' && mediaUrl && <img src={mediaUrl} className="w-full h-full object-cover opacity-60" alt="" />}
          {slide.type === 'video' && mediaUrl && <video src={mediaUrl} className="w-full h-full object-cover opacity-60" autoPlay loop muted playsInline />}
