...
export default function AuthPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-3 sm:p-8 bg-linear-to-b from-slate-900 to-slate-950 text-slate-100">
            <div
                className="w-full max-w-md bg-white/5 border border-white/5 p-7 rounded-2xl shadow-lg"
                role="main"
                aria-labelledby="auth-heading"
            >
                <h1 id="auth-heading" className="text-2xl font-bold">
                ...
                <div className="mt-4 flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                        {mode === "signin" ? (
                            <CreateAccountBtn setMode={handleModeChange} />
                        ) : (
                            <SignInBtn setMode={handleModeChange} />
                        )}
                    </div>
...
function Alert({ kind = "error", children }) {
    ...
    if (kind === "error")
        return (
            <div
                className={`${base} bg-rose-700/20 text-rose-100 border-rose-700/30`}
                role="alert"
            >
                {children}
            </div>
    ...
    return (
        <div
            className={`${base} bg-emerald-600/10 text-emerald-200 border-emerald-600/20`}
            role="status"
        >
            {children}
        </div>
...
function SubmitButton({ loading, mode }) {
    return (
        <button
            className="mt-2 w-full py-2 rounded-lg bg-linear-to-r from-teal-400 to-blue-500 hover:text-white hover:brightness-125 transition-all duration-150 cursor-pointer text-slate-900 font-semibold disabled:opacity-60"
            type="submit"
            disabled={loading}
        >
            {loading
...
function CreateAccountBtn({ setMode }) {
    return (
        <div className="flex flex-col justify-center items-center w-full">
            ...
            <button
                className="text-teal-200 text-sm font-semibold cursor-pointer hover:underline"
                type="button"
...
function SignInBtn({ setMode }) {
    return (
        <>
            ...
            <button
                className="text-teal-200 font-semibold cursor-pointer hover:underline"
                type="button"
